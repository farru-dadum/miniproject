...
app.get("/get-user-score", verifyToken, async (req, res) => { // Use base route, get username from token
  const username = req.user.username; // Get username from verified token
  console.log(` M Fetching score/coupon info for user: ${username}`);

  try {
    // Fetch score details from Supabase 'reward' table
    const { data: reward, error: rewardError } = await supabase
      .from("reward")
      .select("score, redeemed_points")
      .eq("username", username) // Use exact match with logged-in user
      .single(); // Expect only one record per user

    if (rewardError) {
        // Differentiate between "not found" and other errors
        if (rewardError.code === 'PGRST116') { // PostgREST code for "single row not found"
             console.warn(` M No reward record found for user: ${username}`);
             return res.status(404).json({ error: "User reward record not found." });
        }
        console.error(`❌ Supabase error fetching reward for ${username}:`, rewardError);
        return res.status(500).json({ error: "Error fetching user reward data." });
    }

    if (!reward) { // Should be caught by .single() error handling, but belts and suspenders
       console.warn(` M No reward data returned for user: ${username} despite no error.`);
       return res.status(404).json({ error: "User reward data unavailable." });
    }


    const userPoints = reward.score || 0;
    const redeemedPoints = reward.redeemed_points || 0;
    let couponImage = null;
    let message = "";

    // Determine the category the user qualifies for based on current score
    const category = getCategory(userPoints);

    // Check if points are sufficient for any category AND if they haven't redeemed at this level yet
    if (category && userPoints > redeemedPoints) {
        console.log(` M User ${username} qualifies for category '${category}' (Score: ${userPoints}, Redeemed: ${redeemedPoints})`);

        // Fetch available coupons for that category from Supabase 'coupons' table
        const { data: coupons, error: couponError } = await supabase
            .from("coupons") // Ensure this table exists
            .select("image_url")
            .eq("category", category); // Match category exactly

        if (couponError) {
            console.error(`❌ Supabase error fetching coupons for category ${category}:`, couponError);
            // Proceed without coupon, but log the error
            message = "Could not retrieve coupons at this time.";
        } else if (coupons && coupons.length > 0) {
            // Select a random coupon from the available ones
            couponImage = coupons[Math.floor(Math.random() * coupons.length)].image_url;
            console.log(` M Selected coupon image for ${username}: ${couponImage}`);

            // IMPORTANT: Update the redeemed_points in the reward table to prevent re-issuing
            // This makes the coupon "claimed" for the current score level.
            const { error: updateError } = await supabase
                .from("reward")
                .update({ redeemed_points: userPoints }) // Set redeemed to current score
                .eq("username", username);

            if (updateError) {
                console.error(`❌ Supabase error updating redeemed_points for ${username}:`, updateError);
                // Coupon image was selected but redemption status update failed. Critical issue.
                // Maybe don't send the couponImage back if the update fails?
                couponImage = null; // Rollback sending the image
                message = "Error updating reward status. Coupon not issued.";
            } else {
                 console.log(` M Updated redeemed_points to ${userPoints} for user ${username}.`);
                 message = "Coupon retrieved successfully!";
            }
        } else {
            console.log(` M No coupons found in Supabase for category: ${category}`);
            message = "Congratulations on your score! No specific coupons available for your category right now.";
        }
    } else if (userPoints <= redeemedPoints && category) {
        console.log(` M User ${username} has already redeemed coupon for score level ${userPoints} (Redeemed: ${redeemedPoints})`);
        message = "You have already redeemed a coupon for your current score level.";
    } else {
        console.log(` M User ${username} does not qualify for a coupon category yet (Score: ${userPoints})`);
        message = "Keep contributing to earn rewards!";
    }

    // Send response
    res.json({
        score: userPoints,
        redeemedPoints: redeemedPoints, // Send redeemed points too for context
        couponImage: couponImage,
        message: message
    });

  } catch (error) {
    console.error(`❌ Internal Server Error fetching score/coupon for ${username}:`, error);
    res.status(500).json({ error: "Internal server error while fetching rewards." });
  }
});
